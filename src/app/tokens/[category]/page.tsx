...
export default function TokenCategoryPage({ params, searchParams }: TokenCategoryPageProps) {
  ...
  let filteredListings = mockListings.filter(
    (listing: Listing) => listing.type === ListingType.TOKEN && listing.category === params.category
  );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex gap-4 mb-6">
        {['trending', 'new', 'top', 'marketcap'].map((sortOption) => (
          <a
            key={sortOption}
            href={`/tokens/${params.category}?sort=${sortOption}`}
      ...
      {filteredListings.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-600">No tokens found in this category.</p>
        </div>
      )}
    </div>
  );
